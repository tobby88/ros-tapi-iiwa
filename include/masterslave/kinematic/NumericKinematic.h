 ...
 * @brief Klasse zur Berechnung der Kinematiken des Gesamtsystems mit 10 Achsen. Die inverse Kinematik wird numerisch
 * mittels eines Least-Square-Problems gelöst
 * Die Klasse berechnet die direkte und inverse Kinematik eines Gesamtsystems bestehend aus LBR iiwa und dem Laparoskop.
 * Es besitzt insgesamt 10 Achsen, da der Trokarpunkt eingehalten werden muss,
 * bleibt noch eine Redundanz mit Dimension 2 zur Optimierung der Gelenkwinkelkonfiguration.
 * Diese können mit verschiedenen Optimierungskriterien, wie Singularitätsvermeidung, Kollisionsvermeidung,
 * Geschwindigkeits- und Beschleunigungsminimierung, sowie einer Winkelwächterfunktion parametriert werden
...
private:
  ...
  /**
   * @fn calcInvKin
   * @brief Berechnung der inversen Kinematik durch numerische Optimierung eines Least-Squares-Problem nach
   ...
   * @param deltaQ Winkeldifferenz zwischen dem vorherigen Iterationsschritt und dem in dem vorherigen Durchlauf
   * bestimmten Gelenkwinkelvektor
   ...
   */
  Eigen::VectorXd trocarMonitoring(Eigen::VectorXd qAct, Eigen::VectorXd deltaQ,
                                   Eigen::MatrixXd& A);  // and RCM is needed
   ...
   * @return Vektor der Gelenkwinkelpotentiale abgeleitet nach den Gelenkwinkeln
   */
  Eigen::MatrixXd minimizeAcceleration(double cycleTime, Eigen::MatrixXd weightMatrix, Eigen::VectorXd deltaQ,
                                       Eigen::MatrixXd& a);
  ...
  bool directKinematicsCallback(masterslave::NumericKinematicDirectKinematics::Request&,
                                masterslave::NumericKinematicDirectKinematics::Response&);
  bool inverseKinematicsCallback(masterslave::NumericKinematicInverseKinematics::Request&,
                                 masterslave::NumericKinematicInverseKinematics::Response&);
   ...
   */
  const double MAX_ANGLES[10] = { 170 * DEG_TO_RAD, 120 * DEG_TO_RAD, 170 * DEG_TO_RAD, 120 * DEG_TO_RAD,
                                  170 * DEG_TO_RAD, 120 * DEG_TO_RAD, 175 * DEG_TO_RAD, 85 * DEG_TO_RAD,
   ...
   */
  const double MAX_ANGLES_SPEED[10] = { 85 * DEG_TO_RAD,  85 * DEG_TO_RAD,  100 * DEG_TO_RAD, 75 * DEG_TO_RAD,
                                        130 * DEG_TO_RAD, 135 * DEG_TO_RAD, 135 * DEG_TO_RAD, 180 * DEG_TO_RAD,
  ...
  /**
   * @var weightVector
   * @brief Gewichtungsvektor, der online per dynamic_reconfigure geändert werden kann. Dieser wird dann zur diagonalen
   * Wichtungsmatrix
   */
