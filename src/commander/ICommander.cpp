...
void ICommander::commandVelocities()
{
  ...
  if (motorAngles(1) <= CRITICAL_PLIERS_ANGLE && motorAngles(1) >= -CRITICAL_PLIERS_ANGLE &&
      motorAngles(0) >= -CRITICAL_PLIERS_ANGLE && motorAngles(0) <= CRITICAL_PLIERS_ANGLE)
  {
  ...
  if (motorAngles(1) <= -CRITICAL_PLIERS_ANGLE &&
      (jointAnglesTar.bottomRows(3)(2) - jointAnglesAct.bottomRows(3)(2)) < 0)
  {
    Q6pVel.data = 0;
  }
  if (motorAngles(0) >= CRITICAL_PLIERS_ANGLE &&
      (jointAnglesTar.bottomRows(3)(2) - jointAnglesAct.bottomRows(3)(2)) > 0)
  {
    Q6nVel.data = 0;
  }
 ...
 * Des Weiteren wird die Membervariable gripper_stop gesetzt, die verhindert, dass die Greifer über die Grenzen hinaus
 * fahren.
 */
