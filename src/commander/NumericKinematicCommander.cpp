...
NumericKinematicCommander::NumericKinematicCommander(ros::NodeHandle &nh, ros::NodeHandle &drNH) : nh_(nh)
{
  ...
  for (int i = 0; i < 7; i++)
  {
    ...
    // Mit boost wird dem Callback die Nummer des Gelenks für den Callback mitgegeben. Damit man einfach erkennen kann,
    // welches Gelenk den Callback auslöst
    lbrJointAngleSub[i] = nh_.subscribe<sensor_msgs::JointState>(
        sstream.str().c_str(), 1, boost::bind(&NumericKinematicCommander::lbrJointAngleCallback, this, _1, i));
    sstream.str(std::string());
  }
...
void NumericKinematicCommander::statemachineThread(const ros::TimerEvent &event)
{
  ...
  switch (state)
  {
    ...
    case FREE:
      if (stateService.exists())
        stateStringMsg.request.state = "GravComp;";
      callBacksCalled = 0;
      // ROS_INFO("FREE");
      break;
    case MOVE_TO_POSE:
...
void NumericKinematicCommander::loop()
{
  ...
  boundingBox = std::move(
      std::unique_ptr<BoundingBox>(new BoundingBox(nh_, TCP, RCM.translation(), boundingBoxSize, rcmDistance)));
  ...
  while (ros::ok() && state == MOVE_TO_POSE)
  {
    ...
    if (!TCP.isApprox(TCP_old) && boundingBox->checkBoundingBoxTCP(TCP))
    {
      ...
      {
        jointAnglesTar = Eigen::VectorXd::Map(inverseKinematicsService.response.jointAnglesTarget.data(),
                                              inverseKinematicsService.response.jointAnglesTarget.size());
      }
      else
      {
        TCP = TCP_old;
