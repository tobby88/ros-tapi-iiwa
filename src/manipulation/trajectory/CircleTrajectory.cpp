...
CircleTrajectory::CircleTrajectory(Eigen::Affine3d startPoint, Eigen::Vector3d RCM, double radius, double zCoord,
                                   double speed, double cycleTime)
{
...
Eigen::Affine3d CircleTrajectory::calculateNextPoint()
{
  ...
  if (pathParameterStart_ <= 1)
  {
    currentPosition.translation() +=
        ((startPositionTrajectory_.translation() - startPosition_.translation()) /
         Eigen::Vector3d(startPositionTrajectory_.translation() - startPosition_.translation()).norm() * cycleTime_ *
         speed_);
    // 0 <= Path parameter <= 1
    pathParameterStart_ +=
        cycleTime_ / Eigen::Vector3d(startPositionTrajectory_.translation() - startPosition_.translation()).norm() *
        speed_;
  ...
  Eigen::Vector3d incrementVector =
      Eigen::Vector3d(radius_ * cos(2 * M_PI * pathParameter_), radius_ * sin(2 * M_PI * pathParameter_), 0);
