...
NumericKinematic::NumericKinematic(ros::NodeHandle &nh) : nh_(nh)
{
  ...
  inverseKinematicsServer =
      nh_.advertiseService("/inverseKinematics", &NumericKinematic::inverseKinematicsCallback, this);
...
bool NumericKinematic::rcmCallback(masterslave::NumericKinematicRCM::Request &req,
                                   masterslave::NumericKinematicRCM::Response &resp)
{
...
bool NumericKinematic::directKinematicsCallback(masterslave::NumericKinematicDirectKinematics::Request &req,
                                                masterslave::NumericKinematicDirectKinematics::Response &resp)
{
...
bool NumericKinematic::inverseKinematicsCallback(masterslave::NumericKinematicInverseKinematics::Request &req,
                                                 masterslave::NumericKinematicInverseKinematics::Response &resp)
{
  bool retVal = true;
  ...
  std::vector<double> jointAnglesTarget(jointAnglesTar.data(),
                                        jointAnglesTar.data() + jointAnglesTar.rows() * jointAnglesTar.cols());
...
Eigen::VectorXd NumericKinematic::calcDirKin(Eigen::VectorXd jointAngles)
{
  ...
  // LBR Kinematik
  T_0_Q1 = buildAffine3d(Eigen::Vector3d(0, 0, LBR_PARAMETERS.L_0_Q1 + LBR_PARAMETERS.L_Q1_Q2),
                         Eigen::Vector3d(0, 0, M_PI), true);
  Eigen::Affine3d T_Q1_Q2 = buildAffine3d(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(M_PI / 2, 0, jointAngles(0)), true);
  Eigen::Affine3d T_Q2_Q3 =
      buildAffine3d(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(-M_PI / 2, 0, jointAngles(1)), true);
  Eigen::Affine3d T_Q3_Q4 = buildAffine3d(Eigen::Vector3d(0, 0, LBR_PARAMETERS.L_Q2_Q3 + LBR_PARAMETERS.L_Q3_Q4),
                                          Eigen::Vector3d(-M_PI / 2, 0, jointAngles(2)), true);
  Eigen::Affine3d T_Q4_Q5 = buildAffine3d(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(M_PI / 2, 0, jointAngles(3)), true);
  Eigen::Affine3d T_Q5_Q6 = buildAffine3d(Eigen::Vector3d(0, 0, LBR_PARAMETERS.L_Q4_Q5 + LBR_PARAMETERS.L_Q5_Q6),
                                          Eigen::Vector3d(M_PI / 2, 0, jointAngles(4)), true);
  Eigen::Affine3d T_Q6_Q7 =
      buildAffine3d(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(-M_PI / 2, 0, jointAngles(5)), true);
  Eigen::Affine3d T_Q7_FL =
      buildAffine3d(Eigen::Vector3d(0, 0, LBR_PARAMETERS.L_Q6_Q7 + LBR_PARAMETERS.L_Q7_FL),
                    Eigen::Vector3d(0, 0, M_PI - 22.5 * DEG_TO_RAD + jointAngles(6)), true);  // Hotfix: 22.5°

  Eigen::Affine3d T_FL_SCH =
      buildAffine3d(Eigen::Vector3d(TOOL_PARAMETERS.X_RCM, TOOL_PARAMETERS.Y_0_Q4, TOOL_PARAMETERS.Z_0_Q4),
                    Eigen::Vector3d(TOOL_PARAMETERS.A_0_Q4 * DEG_TO_RAD, TOOL_PARAMETERS.B_0_Q4 * DEG_TO_RAD,
                                    TOOL_PARAMETERS.C_0_Q4 * DEG_TO_RAD),
                    true);

  Eigen::Affine3d T_FL_Q8 =
      buildAffine3d(Eigen::Vector3d(TOOL_PARAMETERS.X_0_Q4, TOOL_PARAMETERS.Y_0_Q4, TOOL_PARAMETERS.Z_0_Q4),
                    Eigen::Vector3d(TOOL_PARAMETERS.A_0_Q4 * DEG_TO_RAD, TOOL_PARAMETERS.B_0_Q4 * DEG_TO_RAD,
                                    TOOL_PARAMETERS.C_0_Q4 * DEG_TO_RAD),
                    true);
  // ROS_INFO_STREAM("T_FL_Q8: \n" << T_FL_Q8.matrix());
  T_FL_Q8.rotate(Eigen::AngleAxis<double>(jointAngles(7), Eigen::Vector3d::UnitZ()));
  Eigen::Affine3d T_Q8_Q9 = buildAffine3d(Eigen::Vector3d::Zero(),
                                          Eigen::Vector3d(90 * DEG_TO_RAD, 0, 90 * DEG_TO_RAD + jointAngles(8)), false);
  // Fehler: Zuerst um Q5 drehen, dann verschieben
  Eigen::Affine3d T_Q9_Q10 =
      buildAffine3d(Eigen::Vector3d(TOOL_PARAMETERS.L_Q5_Q6, 0, 0), Eigen::Vector3d(-90 * DEG_TO_RAD, 0, 0), true);
  Eigen::Affine3d T_Q10_EE =
      buildAffine3d(Eigen::Vector3d::Zero(), Eigen::Vector3d(0, -M_PI / 2, jointAngles(9)), true);
...
bool NumericKinematic::calcInvKin(Eigen::Affine3d T_0_EE)
{
  ...
  bieq.head(10) = URSULA_MAX_ANGLES_SPEED * cycleTime * (maxSpeed)*3;
  bieq.tail(10) = URSULA_MAX_ANGLES_SPEED * cycleTime * (maxSpeed)*3;
  ...
  while (residualNorm <= residualOld)
  {
    ...
    Eigen::VectorXd bkin = -calcDirKin(jointAnglesIterationPrevious) -
                           calcAnalyticalJacobian(jointAnglesIterationPrevious) * deltaQ + desiredEEPosition;
     ...
     * - APPROXIMIERTE (soweit möglich) Einhaltung X und Y am TCP; keine exakte Einhaltung, da die
     * Gleichungsnebenbedingungen sonst linear abhängig sein können/sind
     ...
     */
    C << tcpGain * cycleTime * cycleTime * Akin.topRows(2), cycleTime * cycleTime * accelerationGain * C_acc,
        cycleTime * cycleTime * velocityGain * C_vel, cycleTime * cycleTime * singularityGain * C_sing.transpose(),
        cycleTime * cycleTime * angleMonitoringGain * C_ang.transpose();
    d << -tcpGain * cycleTime * cycleTime * bkin.topRows(2), -cycleTime * cycleTime * accelerationGain * d_acc,
        -cycleTime * cycleTime * velocityGain * d_vel, -cycleTime * cycleTime * singularityGain * d_sing,
        -cycleTime * cycleTime * angleMonitoringGain * d_ang;
    ...
    try
    {
      residual = Eigen::solve_quadprog(copyCTC, copyCTD, Eigen::MatrixXd(Aeq), Eigen::VectorXd(beq),
                                       Eigen::MatrixXd(Aieq), Eigen::VectorXd(bieq), dQIteration, 1);
    }
    ...
    if (residualNorm == std::numeric_limits<double>::infinity() || iterations >= maxIterations ||
        std::abs(residualNorm - residualOld) < 1e-10)
      break;
...
NumericKinematic:
  ...
  for (int i = 0; i < q.rows(); i++)
  {
    ...
    else if (-URSULA_MAX_ANGLES(i) <= q(i) && critical * -URSULA_MAX_ANGLES(i) >= q(i))
    {
      // Critical Zone
      H +=
          Hmax / 2 * (1 - cos(M_PI * (q(i) + critical * URSULA_MAX_ANGLES(i)) / (1 - critical) * URSULA_MAX_ANGLES(i)));
      dH(i) = -M_PI * Hmax / (2 * ((1 - critical) * URSULA_MAX_ANGLES(i))) *
              sin(M_PI * (q(i) + critical * URSULA_MAX_ANGLES(i)) / (1 - critical) * URSULA_MAX_ANGLES(i));
    }
    ...
    else if (critical * URSULA_MAX_ANGLES(i) <= q(i) && URSULA_MAX_ANGLES(i) >= q(i))
    {
      ...
      H +=
          Hmax / 2 * (1 - cos(M_PI * (q(i) - critical * URSULA_MAX_ANGLES(i)) / (1 - critical) * URSULA_MAX_ANGLES(i)));
      dH(i) = M_PI * Hmax / (2 * ((1 - critical) * URSULA_MAX_ANGLES(i))) *
              sin(M_PI * (q(i) - critical * URSULA_MAX_ANGLES(i)) / (1 - critical) *
                  URSULA_MAX_ANGLES(i));  // Differentiation: dH
    }
...
Eigen::MatrixXd NumericKinematic::minimizeAcceleration(double cycleTime, Eigen::MatrixXd weightMatrix,
                                                       Eigen::VectorXd deltaQ, Eigen::MatrixXd &a)
{
...
Eigen::VectorXd NumericKinematic::trocarMonitoring(Eigen::VectorXd qAct, Eigen::VectorXd deltaQ, Eigen::MatrixXd &A)
{
  ...
  // --> Stichwort: Schnittpunkt Gerade - Ebene
  ...
  Eigen::VectorXd ages = -(shaftPositionRPY + Ages * deltaQ - trocarPositionRPY);  // - 0-Vektor im Trokarsystem da der
                                                                                   // gewünschte Punkt [0 0 0] im
                                                                                   // Trokar-KS ist.
...
Eigen::VectorXd NumericKinematic::avoidSingularities(Eigen::VectorXd qAct, Eigen::VectorXd qOffset, double weight,
                                                     double &a)
{
