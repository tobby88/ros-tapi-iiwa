...
RosOpenIgtlBridge::RosOpenIgtlBridge(ros::NodeHandle nh) : nh_(nh)
{
  ...
  for (int i = 0; i < lbrJointAngleSub.size(); i++)
  {
    ...
    lbrJointAngleSub[i] = nh_.subscribe<std_msgs::Float64>(
        sstream.str().c_str(), 1, boost::bind(&RosOpenIgtlBridge::lbrJointAngleCallback, this, _1, i));
...
bool RosOpenIgtlBridge::stateService(masterslave::OpenIGTLStateService::Request &req,
                                     masterslave::OpenIGTLStateService::Response &res)
{
  std::stringstream sstream;
  stateUpdateMutex_.try_lock();
  if (rTransform == -1)
